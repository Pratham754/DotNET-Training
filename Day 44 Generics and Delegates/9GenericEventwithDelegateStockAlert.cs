...
public class ThresholdMonitor<T> where T : IComparable<T>
{
    ...
    public void Update(T newValue)
    {
        ...
        // 2) Update _current
        bool reached = newValue.CompareTo(_threshold) >= 0 && _current.CompareTo(_threshold) < 0;
        if (reached)
        {
            var values = new ThresholdChangedEventArgs<T>
            {
                OldValue = _current,
                NewValue = newValue,
                Message = $"Threshold {_threshold} reached."
            }; 
            ThresholdCrossed?.Invoke(this, values);
        }
        _current = newValue;
    }
}
